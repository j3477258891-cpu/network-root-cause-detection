"""Pure additions on single-root orders; require all three models in Top-20."""
from __future__ import annotations
import csv,gzip,hashlib,json,sys
from pathlib import Path
import numpy as np
ROOT=Path(r"D:\zgyidong");EXP=ROOT/"experiments";BASE=EXP/"v124_campaign/v124_single_pair_015_from_probe13.csv";DATA=EXP/"v25_semantic_router/cloud_dataset/v25_semantic_router.npz";REC=EXP/"v25_semantic_router/cloud_dataset/semantic_records.json.gz";OUT=EXP/"v139_unanimous_top20_additions";MODELS=['station_extra_trees','template_extra_trees','v30_consensus']
...
def main():
 ...
 arr={n:np.load(EXP/'v30_meta_stack'/f'{n}_test.npy') for n in MODELS};ban=blocked(); top={}
 for n in MODELS:
  rows=[]
  for i,o in enumerate(rec):
   oid=o['order_id'];cur={x['@rid'] for x in base[oid]}
   if len(cur)!=1:continue
   c=[(float(arr[n][int(ptr[i])+j]),a['rid']) for j,a in enumerate(o['alarms']) if a['rid'] not in cur and (oid,a['rid']) not in ban]
   if c:rows.append((max(c),oid))
  rows.sort(reverse=True);top[n]=rows[:20]
 sets={n:{(oid,rid) for (sc,rid),oid in top[n]} for n in MODELS};common=set.intersection(*sets.values());props=[]
 for oid,rid in common:
  vals={n:next(sc for (sc,x),o in top[n] if o==oid and x==rid) for n in MODELS};props.append({'order_id':oid,'add_rid':rid,'mean_score':float(np.mean(list(vals.values()))),'model_scores':vals})
 ...
 stem=f'v139_unanimous_top20_single_root_add_top{len(chosen)}_from_pair015';path=outdir/f'{stem}.csv'
 ...
 meta={'version':'v139-unanimous-top20-single-root-addition','base':str(BASE),'base_sha256':sha(BASE),'models':MODELS,'topn_each':20,'restriction':'base_root_count == 1','available_unanimous':len(props),'selected':chosen,'predictions':total,'orders':len(order),'sha256':sha(path),'oof_reference':'three-model single-root top20 unanimous: 9/10 true','status':'exploratory_not_scored','warning':'OOF/model evidence only; no online score yet.'}
