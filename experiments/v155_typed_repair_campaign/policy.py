"""V155 two-step conditional working policy; exact U/score arithmetic, not target confidence."""
...
WARNING = ('Equal-prior model mixture with independent action outcomes within each model, '
           ...
           'Not an exact global label posterior, calibrated target probability, or six-step global optimum.')
...
class Feasibility:
    ...
    def __init__(self, eq, byid):
        ...
        capacities = np.zeros(len(groups))
        rows = [dict() for _ in eq.rows]
        for col, j in mapping.items():
            capacities[j] += 1
        for (pattern, _), j in groups.items():
            for r, value in pattern:
                rows[r][j] = value
...
class Policy:
    ...
    def best(self, leaves):
        ...
        if key not in self.best_cache:
            self.best_cache[key] = c.best_union(leaves, self.byid)
    ...
    def options(self, leaves, attempted):
        ...
        for leaf in leaves:
            ...
            if leaf['delta_tp'] in (sum(min(s) for s in supports), sum(max(s) for s in supports)):
                continue
            if all(sum(p > 0 for p in self.dist(ids, f).values()) == 1 for f in self.families):
                continue
            seen = set()
    ...
    def risk(self, leaves, champion, attempted):
        ...
        for ids in sorted(variants):
            if not ids or ids in attempted:
                continue
            ...
            for family, weight in weights.items():
                ...
                if allowed:
                    for dt, p in normalize(allowed).items():
                        mixed[dt] += weight * p
            if not mixed:
                continue
