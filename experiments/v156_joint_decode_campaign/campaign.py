...
class Search:
    ...
    def first_query(self, new_mask, budget):
        ...
        vec, pos, priors = self.new_vec, self.new_pos, self.new_priors
        # default is the guaranteed merge (this is already the terminal when nothing
        # further is worth probing, and the correct value when already decoded).
        floor = self._floor(new_mask, self.s2_full)
        best = {"val": floor, "q": None, "worst": floor, "branches": []}
        for q in roots:
            ps = tuple(pos[i] for i in q)
            groups = defaultdict(lambda: [0.0, 0])
            for j in range(len(vec)):
            ...
            if self.truncated:
                break
        if self.truncated:
            # a timed-out search yields no validated strategy: return the floor only
            return {"val": floor, "q": None, "worst": floor, "branches": [], "truncated": True}
        return best
...
def load_csv(path, strict=True):
    # Platform reads a plain-UTF-8 header; a BOM would surface as "\ufefforder_id".
    # Read with plain utf-8 (never utf-8-sig, which silently strips the BOM) and
    # reject any BOM explicitly so the defect can never hide again.
    require(not Path(path).read_bytes().startswith(b"\xef\xbb\xbf"),
            f"UTF-8 BOM forbidden (platform expects a plain-UTF-8 header): {path}")
    with Path(path).open(encoding="utf-8", newline="") as fh:
        reader = csv.DictReader(fh)
        require(reader.fieldnames == ["order_id", "output"],
                f"Invalid CSV header (plain UTF-8 required): {path} -> {reader.fieldnames}")
        rows = list(reader)
    ids, roots, nodes = [], {}, {}
    for row in rows:
        oid = row["order_id"]
...
def validate_csv(path, action_ids, expected_sha=None):
    require(not Path(path).read_bytes().startswith(b'\xef\xbb\xbf'), 'UTF-8 BOM is forbidden in submission CSV')
...
def author(query_ids, carrier):
    ...
    # never emit a second file for an identical action set
    same = next((e for e in manifest["files"]
                 if sorted(e.get("all_ids", [])) == all_ids), None)
    if same:
        return same
...
if __name__ == "__main__":
    try:
        # The legacy search above is retained for provenance, not used for live decisions.
        from verified_engine import main as verified_main
        verified_main()
